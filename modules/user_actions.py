...
class UserActions(commands.Cog):
  ...
  async def stats(self, interaction: discord.Interaction):
    ...
    embed = discord.Embed(title=f"Stats for {interaction.user.name}!", colour=discord.Colour.dark_gold())
    embed.add_field(name='XP', value=f'Level: {user.level}\nXP: {user.xp}\nXP Step: {user.xp_step}\nXP Next: {user.xp_next}', inline=False)
    embed.add_field(name='Coins', value=f'Coins: {user.coins}\nDaily: {daily_status}', inline=False)
  ...
  async def daily(self, interaction: discord.Interaction):
    ...
    if now < next_claim:
      embed = discord.Embed(title="Dailies!", description="Hold up!\nYour time isn't up yet!", colour=discord.Colour.red())
    ...
    total_levels, total_coins = self.bot.db.add_xp(guild_id= guild_id, member_id= member_id, xp=self.bot.db.DAILY_XP_BONUS, user= user)

    user.lastclaimed = datetime.now()

    self.bot.db.update_user(guild_id= guild_id, member_id= member_id, user= user)

    embed = discord.Embed(title="Dailies!", description="Here's your daily reward!", colour=discord.Colour.green())
    embed.add_field(name='Rewards', value=f'Coins: {self.bot.db.DAILY_BONUS_COINS}\nXP: {self.bot.db.DAILY_XP_BONUS}')

    if total_levels != 0:
      embed.add_field(name='You leveled up!', value=f'Earned Coins: {total_coins}\nEarned Levels: {total_levels}')
